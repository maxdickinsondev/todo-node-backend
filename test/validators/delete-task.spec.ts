...
describe("DeleteTask Validator", () => {
  beforeEach(() => {
    app.delete("/delete_task/:id", validator(updateTaskSchema), (req, res) => {
      res.status(200).send("Success!");
    });
  });

  it("should be return 200 if task is valid", async () => {
    const response = await supertest(app).delete("/delete_task/1");
    expect(response.status).toBe(200);
    expect(response.text).toBe("Success!");
  });

  it("should be return 400 if id is missing", async () => {
    const response = await supertest(app).delete("/delete_task/any");
